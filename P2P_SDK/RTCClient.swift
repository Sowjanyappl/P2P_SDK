...
public protocol RTCClientDelegate: class {
    ...
    func rtcClient(client : RTCClient, didReceiveLocalVideoTrack localVideoTrack: RTCVideoTrack, peer:RTCPeerConnection)
    func rtcClient(client : RTCClient, didReceiveRemoteVideoTrack remoteVideoTrack: RTCVideoTrack,peer:RTCPeerConnection)
...
public class RTCClient: NSObject {
    ...
    public func startConnection() {
        ...
        if let localVideoTrack = localStream.videoTracks.first {
            self.delegate?.rtcClient(client: self, didReceiveLocalVideoTrack: localVideoTrack, peer:peerConnection)
        }
       ...
       print("stream data",stream)
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            if stream.videoTracks.count > 0 {
                self.delegate?.rtcClient(client: self, didReceiveRemoteVideoTrack: stream.videoTracks[0], peer:peerConnection)
            }
        }
